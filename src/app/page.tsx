...
export default function AgencyLandingPage() {
  ...
  return (
    <div className="relative w-full overflow-x-hidden selection:bg-amber-500/20 selection:text-amber-300">
      ...
      {/* Sticky Top Nav */}
      <header className="sticky top-0 z-50 w-full backdrop-blur-xl bg-[#08090D]/80 border-b border-white/[0.08] transition-all duration-300">
        <div className="max-w-[1400px] mx-auto px-4 md:px-8 py-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2 p-1 rounded-full bg-black/40 border border-white/5">
              <button 
                onClick={() => setLang('en')}
                className={`px-3 py-1 text-[11px] font-mono font-medium rounded-full transition-all duration-300 ${lang === 'en' ? 'bg-amber-500 text-black' : 'text-slate-400 hover:text-white'}`}
              >
                EN
              </button>
              <button 
                onClick={() => setLang('es')}
                className={`px-3 py-1 text-[11px] font-mono font-medium rounded-full transition-all duration-300 ${lang === 'es' ? 'bg-amber-500 text-black' : 'text-slate-400 hover:text-white'}`}
              >
                ES
              </button>
            </div>
      ...
      <main className="relative z-10 w-full pt-16 pb-20 md:pt-24 md:pb-24 px-4 md:px-8 max-w-[1400px] mx-auto">
...
{`{
  ...
  "action_chain": [
    ...
    "whatsapp_agent_alert"
        ...
        {/* Audit Form */}
        <section id="audit" className="py-16 md:py-20">
